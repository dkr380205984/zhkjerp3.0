            ...
            <div class="label">
              ...
              <el-tooltip class="item" effect="dark" content="设置成功后请点击此按钮刷新数据" placement="top">
                <i class="el-icon-refresh hoverGreen fr"
                  style="line-height:38px;font-size:18px;margin-left:8px;cursor:pointer" @click="getStaffList()"></i>
              </el-tooltip>
              <el-tooltip class="item" effect="dark" content="添加报销人" placement="top">
                <i class="el-icon-upload hoverOrange fr"
                  style="line-height:38px;font-size:18px;margin-left:8px;cursor:pointer"
                  @click="$openUrl('/workerManage/create')"></i>
              </el-tooltip>
            </div>
            <div class="info elCtn">
              <el-select v-model="receiptInfo.staff" filterable placeholder="请选择报销人" value-key="id">
                <el-option v-for="item in staffList" :key="item.id" :label="item.code.substr(-4) + ' ' + item.name"
          ...
          <div class="col">
            ...
            <div class="info elCtn">
              <el-select v-model="receiptInfo.group" placeholder="请选择所属小组" clearable>
                <el-option v-for="item in groupList" :key="item.name" :label="item.name" :value="item.id">
                </el-option>
              </el-select>
            </div>
          </div>
            ...
            <div class="listCtn">
              <div class="list">
                ...
                <div class="row" v-for="(item, index) in receiptInfo.staff_departments" :key="index">
                  ...
                  <div class="col">
                    <div>
                      <span class="opr hoverBlue" style="top: 20%" @click="
                        $addItem(receiptInfo.staff_departments, {
                          name: '',
                          amount: ''
                        })
                      ">添加</span>
                      <span class="opr hoverRed" style="top: 20%; left: 20%"
        ...
        <div class="row">
          <div class="col">
            <div class="label">
              ...
              <el-checkbox v-model="cvFlag" @change="changeCVOpration">{{ cvFlag ? '关闭复制粘贴图片上传功能' : '开启复制粘贴图片上传功能' }}
              </el-checkbox>
            </div>
            <div class="info">
              ...
              <div class="fileCtn">
                <el-upload class="upload" action="https://upload.qiniup.com/"
                  accept="image/jpeg,image/gif,image/png,image/bmp" :before-upload="beforeAvatarUpload" :data="postData"
                  :file-list="file_list" :on-remove="removeFile" :on-success="successFile"
                  :on-preview="handlePictureCardPreview" ref="uploada" list-type="picture-card">
                  <div class="uploadBtn">
                    <i class="el-icon-upload"></i>
                    <span>上传图片</span>
                  </div>
                  <div slot="tip" class="el-upload__tip">只能上传jpg/png图片文件，且不超过10M(请勿上传带特殊字符的图片)</div>
                </el-upload>

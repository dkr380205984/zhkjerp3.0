<template>
  <div id="payTimeList" v-loading="loading" class="bodyContainer">
    <div class="module">
      ...
      <div class="listCtn">
        <div class="filterCtn">
          ...
          <div class="elCtn" style="position: relative">
            <el-select style="width: 95%" @change="changeDepartment()" v-model="department" placeholder="部门筛选" clearable>
              <el-option v-for="(item, index) in departmentList" :key="index" :value="item.id"
          ...
          <div class="elCtn">
            <el-cascader v-model="process" :options="processList" @change="changeRouter" :show-all-levels="false"
              filterable placeholder="工序筛选" clearable></el-cascader>
          </div>
          <div class="elCtn">
            <el-date-picker v-model="date" type="daterange" align="right" unlink-panels range-separator="至"
              start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions" @change="changeRouter"
        ...
        <div class="filterCtn clearfix">
          ...
          <div class="btn backHoverBlue fr" style="margin-bottom: 20px; margin-left: 20px"
        ...
        <div class="tableCtn" style="padding-left: 0; padding-right: 0; padding-top: 0">
          <el-tabs v-model="activeName" type="card" @tab-click="getList">
            <el-tab-pane label="计件结算表" name="1">
              <el-table ref="multipleTable" :data="list" tooltip-effect="dark" :row-key="rowKey" style="width: 100%"
                ...
                <el-table-column label="工序说明" width="110" fixed>
                  <template slot-scope="scope">
                    <div v-if="scope.row.process_desc">
                      ...
                      <el-tooltip v-else class="item" effect="dark" :content="scope.row.process_desc || '无工序说明'"
                        placement="top-start">
                ...
                <el-table-column prop="product_code" label="产品编号" width="160">
                  <template slot-scope="scope">
                    <span class="blue" style="cursor: pointer" @click="productDetail = scope.row.product_id
                                                                productShow = true
                ...
                <el-table-column label="次品原因" width="120">
                  <template slot-scope="scope">
                    <el-tooltip class="item" effect="dark" v-if=" scope.row.shoddy_reason "
                      :content=" scope.row.shoddy_reason " placement="top-start">
                      <span class="blue" style="cursor: pointer">查看</span>
                    </el-tooltip>
            ...
            <el-tab-pane label="其它计价结算表" name="2">
              <el-table ref="multipleTable2" :data=" list " tooltip-effect="dark" :row-key=" rowKey " style="width: 100%"
                @selection-change=" handleSelectionChange ">
                <el-table-column type="selection" width="55" :reserve-selection=" true "></el-table-column>
                <el-table-column prop="id" label="序号" width="70" fixed></el-table-column>
                <el-table-column prop="staff_name" label="员工姓名" width="130" fixed>
        ...
        <div class="pageCtn">
          <el-pagination background :page-size=" limit " layout="prev, pager, next, jumper" :total=" total "
            :current-page.sync=" page " @current-change=" changeRouter ">
          </el-pagination>
        </div>
      </div>
    </div>
    ...
    <product-detail :id=" productDetail " :show=" productShow " @close=" productShow = false "
      :noOpr=" true "></product-detail>
  </div>
</template>
...
export default Vue.extend({
  ...
  methods: {
    getFilters() {
      ...
      this.date =
      // @ts-ignore
        query.date ? query.date.split(',') : [new Date().getFullYear() +
          '-' +
          (new Date().getMonth() + 1 < 10 ? '0' + (new Date().getMonth() + 1) : new Date().getMonth() + 1) + '-' + '01',
        new Date().getFullYear() +
        '-' +
        (new Date().getMonth() + 1 < 10 ? '0' + (new Date().getMonth() + 1) : new Date().getMonth() + 1) + '-' + new Date().getDate()]

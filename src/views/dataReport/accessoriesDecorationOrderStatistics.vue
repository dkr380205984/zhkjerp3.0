<template>
  <div id="accessoriesDecorationOrderStatistics" class="bodyContainer" v-loading="loading">
      ...
      <div class="cardCtn">
        ...
        <div class="card" style="margin-left: 4px">
          ...
          <div class="contentGrid">
            ...
            <div>
              辅料名称：<span class="blue">{{ filterData.name || '所有' }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bottomFixBar">
      <div class="main">
        <div class="btnCtn">
          <div class="btn backHoverBlue" @click="$router.push('/billingManagement/auxiliaryMaterialPurchaseOrder')">查看辅料订购单</div>
        </div>
      </div>
    </div>
  </div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    changeRouter() {
      this.$router.push(
        '/dataReport/accessoriesDecorationOrderStatistics?' +
          '&client_id=' +

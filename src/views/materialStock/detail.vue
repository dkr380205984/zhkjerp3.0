                  ...
                  <div class="trow"
                    ...
                    <div class="tcol">
                      {{
                        ...
                        itemChild.process_info
                          .filter((item) => item.process === '染色')
                          .reduce((total, cur) => total + Number(cur.number), 0)
                      }}{{ itemChild.unit || 'kg' }}
                    </div>
                  </div>
                </div>
            ...
            <div class="tcol"
              ...
              <span>{{ item.is_check | filterCheck }}
                <el-tooltip v-if="item.is_check === 4"
                  class="item"
                  effect="dark"
                  content="：由于【入库数量】超过了【订购数量】的10%。该原料入库单已变为异常状态。以下为异常单据处理办法：1. 检查实际入库数量，如果数量录入错误可删除单据，再重新创建入库单。2. 如果录入的数量为实际入库数量，则无需操作，或点击审核通过即可。"
                  placement="top">

      ...
      <div class="listCtn">
          ...
          <div v-for="(item, index) in list" :key="index">
            <div class="row">
              ...
              <div
                ...
                @click="$router.push('/boxManage/boxDetail?id=' + item.id)"
              >
                {{ item.code }}
              </div>
                      ...
                      <div class="trow" v-for="(itemData, indexData) in itema.data" :key="indexData">
                        <div class="tcol noPad" style="flex: 4">
                          <div class="trow" v-for="(itemPro, indexPro) in itemData.product_info" :key="indexPro">
                            ...
                            <div class="tcol">{{ $toFixed(itemPro.pack_number_all, 3, true) }}</div>
                            <div class="tcol">{{ $toFixed(itemPro.transport_number, 3, true) }}</div>
                          </div>
                        </div>
                        <div class="tcol" style="flex: 0.5">{{ itemData.total_box_count }}箱</div>
                        <div class="tcol" style="flex: 0.5">{{ $toFixed(itemData.total_gross_weight, 3, true) }}kg</div>
                        <div class="tcol" style="flex: 0.5">{{ $toFixed(itemData.total_net_weight, 3, true) }}kg</div>
                        <div class="tcol" style="flex: 0.5">{{ $toFixed(itemData.total_bulk, 3, true) }}m³</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="tableCtn noPadBtm" style="padding-left: 0; padding-right: 0">
                  ...
                  <div class="tbody">
                    <div class="trow">
                      ...
                      <div class="tcol">{{ $toFixed(itemBatch.total_box_count, 3, true) }}箱</div>
                      <div class="tcol">{{ $toFixed(itemBatch.total_gross_weight, 3, true) }}kg</div>
                      <div class="tcol">{{ $toFixed(itemBatch.total_net_weight, 3, true) }}kg</div>
                      <div class="tcol">{{ $toFixed(itemBatch.total_bulk, 3, true) }}m³</div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="tableCtn">
                ...
                <div class="tbody">
                  <div class="trow" v-for="(item, index) in item.detail.batchInfo" :key="index">
                    ...
                    <div class="tcol">{{ $toFixed(item.transport_number, 3, true) }}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div style="margin-top: 20px">
          <span style="line-height: 35px; margin-left: 40px">
            合计运输立方：
            <span class="green" style="font-weight: bold"> {{ $toFixed(additional.total_number, 3, true) }} 立方 </span>
          </span>
          <span style="line-height: 35px; margin-left: 40px">
            合计运输金额：
...
<script lang="ts">
import Vue from 'vue'
import { order, listSetting, client, boxManage, check } from '@/assets/js/api'
...
export default Vue.extend({
  ...
  methods: {
    ...
    changeRouter(ev?: any) {
      ...
      this.$router.push(
        '/billingManagement/transportationDeliveryOrder?page=' +
          this.page +
          '&keyword=' +
          this.keyword +
          '&client_id=' +
        ...
        boxManage
          ...
          .then((ress) => {
            if (ress.status) {
              ...
              // @ts-ignore
              let packPlanLog = data.rel_plan
              packPlanLog.forEach((itemPack: any) => {
                itemPack.transport_info = JSON.parse(itemPack.transport_info)
                itemPack.plan_info.data.forEach((itemData: any) => {
                  itemData.total_box_count = itemData.info_data.reduce((total: any, cur: any) => {
                    return total + Number(cur.box_count)
                  }, 0)
                  itemData.total_bulk = itemData.info_data.reduce((total: any, cur: any) => {
                    return total + Number(cur.total_bulk)
                  }, 0)
                  itemData.total_gross_weight = itemData.info_data.reduce((total: any, cur: any) => {
                    return total + Number(cur.total_gross_weight)
                  }, 0)
                  itemData.total_net_weight = itemData.info_data.reduce((total: any, cur: any) => {
                    return total + Number(cur.total_net_weight)
                  }, 0)
                  itemData.product_info.forEach((itemPro: any) => {
                    itemPro.pack_number_all = 0
                    itemData.info_data.forEach((itemInfo: any) => {
                      itemInfo.product_info.forEach((itemChild: any) => {
                        if (
              ...
              // @ts-ignore
              data.packPlanLogCopy = packPlanLog.map((item) => item.transport_info)
              // @ts-ignore
              data.batchInfo = data.rel_batch
              data.orderBatchCopy = data.order_transport_info ? JSON.parse(data.order_transport_info).batch_data : []
...
<style lang="less">
@import '~@/assets/css/billingManagement/transportationDeliveryOrder.less';
</style>    
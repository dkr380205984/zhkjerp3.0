          ...
          <div v-for="(item, index) in list"
            ...
            <div class="row">
              ...
              <div class="col hoverBlue"
                ...
                @click="$router.push('/settlement/collectionDetail?id='+item.client_id)">{{ item.code || '无' }}</div>
              <div class="col hoverBlue"
                style="
                  cursor: pointer;
                  white-space: nowrap;
              ...
              <div class="col"
                ...
                <el-tooltip class="item"
                  effect="dark"
                  :content="item.reason ? item.reason.toString() : '无'"
                  placement="top-start">
                  <div>{{ item.reason ? item.reason.toString() : '无' }}</div>
                </el-tooltip>
              </div>
              <div class="col">
            ...
            <div v-show="item.isShow"
              style="border: 1px solid #e8e8e8; transform: translateY(-1px)">
              <div class="titleCtn">
      ...
      <div class="main">
        ...
        <div class="contentCtn">
          ...
          <div class="row">
            ...
            <div class="info">
              <el-cascader placeholder="筛选扣款单位 "
...
<script lang="ts">
import Vue from 'vue'
import { order, listSetting, client, deduct, check } from '@/assets/js/api'
...
export default Vue.extend({
  ...
  methods: {
    ...
    changeRouter(ev?: any) {
      ...
      this.$router.push(
        '/billingManagement/deductionForm?page=' +
          this.page +
          '&keyword=' +
          this.keyword +
          '&client_id=' +
    ...
    changeShow(item: any) {
      ...
      if (item) {
        if (item.doc_type === 2) {
          this.$router.push('materialPlan/detail?id=' + item.order_id)
        }
        if (item.doc_type === 4) {
          this.$router.push('/productionPlan/detail?id=' + item.order_id)
        }
      }
    },
    getListSetting() {
...
<style lang="less">
@import '~@/assets/css/billingManagement/deductionForm.less';
</style>    
        ...
        <div class="list">
          ...
          <div v-for="(item, index) in list" :key="index">
            <div class="row">
              ...
              <div v-if="item.plan_id" class="col hoverBlue" style="flex: 1.2;cursor:pointer" @click="$router.push('/materialManage/detail?id='+item.plan_id)">{{ item.code }}</div>
              <div v-if="item.reserve_id" class="col hoverBlue" style="flex: 1.2;cursor:pointer" @click="$router.push('/materialPlanOrder/detail?id='+item.reserve_id)">{{ item.code }}</div>
              <div v-if="item.sup_id" class="col hoverBlue" style="flex: 1.2;cursor:pointer" @click="$router.push('/materialManage/detail?id='+item.sup_id)">{{ item.code }}</div>
              <div
                class="col hoverBlue"
                style="
                  cursor: pointer;
              ...
              <div class="tableCtn" style="padding-top: 0">
                ...
                <div class="tbody">
                  <div class="trow" v-for="(itemChild, indexChild) in item.detail.info_data" :key="indexChild">
                    ...
                    <div class="tcol">{{ itemChild.settle_price ? itemChild.settle_price + '元' : '未填写' }}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div style="margin-top: 20px">
          <span style="line-height: 35px; margin-left: 40px">
            ...
            <span class="green" style="font-weight: bold">
              {{ (additional.total_order_number / 1000).toFixed(2) }} 吨或千米
            </span>
          </span>
          <span style="line-height: 35px; margin-left: 40px">
            合计订购金额：
